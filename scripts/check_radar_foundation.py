...
def main() -> int:
    ...
    try:
        ...
        expected = {"forkit_core-0.1.0-py3-none-any.whl", "forkit_radar-0.1.0b3-py3-none-any.whl"}
        ...
        wheel_requirements.write_text(
            ...
            + "\nforkit-radar==0.1.0b3 --hash=sha256:"
            + report["wheel_sha256"]["forkit_radar-0.1.0b3-py3-none-any.whl"]
            + "\n"
        )
        install_projects = [
            "-m",
        ...
        rebuilt = rebuild / "forkit_radar-0.1.0b3-py3-none-any.whl"
        ...
        run("offline-bundle-install", ["/bin/sh", str(installer.with_name("install.sh")), "--prefix", str(application), "--bin-dir", str(run_dir / "bundle-bin")], outside)
