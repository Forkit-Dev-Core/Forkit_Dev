...
<pre id="install">sh install.sh --no-app</pre><button data-copy="install">Copy install command</button>
...
<p>If an old installation exists, keep it and its history. Install this candidate into separate new application and command directories:</p><pre>sh install.sh --no-app --prefix "$HOME/.local/share/forkit-radar-beta1" --bin-dir "$HOME/.local/bin/forkit-beta1"</pre>
...
<section><p>Installation enables user hooks for detected tools. No repository connection. Open private history with <code>~/.local/bin/forkit-radar open</code>. Run <code>setup</code> again if you install another coding tool later. Choose <code>sh install.sh --no-capture</code> to skip automatic integration.</p><h2>2. Capture your first real coding session</h2><p>In Terminal, go to the root of your existing Git project. Do not initialize an unrelated parent folder.</p>
<pre id="doctor">~/.local/bin/forkit-radar doctor</pre><button data-copy="doctor">Copy local setup check</button>
<h3>Automatic official hooks</h3><pre id="hooks">~/.local/bin/forkit-radar setup --status</pre><button data-copy="hooks">Copy hook setup</button><p>Review Forkit’s user hooks in Codex with <code>/hooks</code>, then start a new session. Forkit merges only its owned entries, keeps backups and stops on conflicting changes. See <a href="CAPTURE.md">CAPTURE.md</a> for exact coverage and recovery. Claude Code and Cursor adapters are separate and experimental.</p>
...
<section><h2>3. See this session, then Today</h2><pre id="review">~/.local/bin/forkit-radar receipt --files
...
<p>After the next session, inspect the new receipt immediately. Tomorrow, open Today again; you do not have to wait a week. Look for what changed since the previous session. User hooks capture later local Git sessions without per-project setup. Open Forkit again for fresh history; no persistent Forkit daemon is installed. Weekly history remains optional, and all local history stays free.</p>
...
<details><summary>Optional: tools and destinations (experimental)</summary><pre>~/.local/bin/forkit-radar setup --activity</pre><p>Review changed hooks in your coding tool and start a new session. Supported MCP events, plus Claude Code WebFetch/WebSearch, can retain tool categories, hostnames and reported outcomes locally. No URL paths, queries, headers, prompts or results are retained. Codex hosted search, hidden script requests and personal browser history are not observed. These are not verified API requests. Read CAPTURE.md for exact limits.</p><pre>~/.local/bin/forkit-radar setup --no-activity</pre><p>Stops future activity collection. Existing history stays free and private.</p></details>
...
<details><summary>Keep your data, remove or change the application</summary><p>Pause automatic capture with <code>forkit-radar setup --disable</code> before removing application files. It removes only Forkit-owned hooks and preserves history. Finish or explicitly recover an active interval first. The installer prints its application and command locations. Removing those application files does not require deleting your session data. Preserve <code>~/.forkit-radar</code> and your existing Core registry. Back up a private store using <code>storage backup --output /new/private/backup.sqlite3</code> before a later version change.</p><p>Application virtual environments depend on the original Python installation and location; do not move them. The installer refuses overwrites. Count reporting stays off until new explicit consent.</p></details>
<div id="copy-status" role="status" aria-live="polite"></div><footer>Local-first by default. This is the CLI kit: check this archive’s exact platform in README.txt. Python and Git prerequisites apply. Native Mac archives are withheld for this CLI-only release. Linux uses the CLI and an offline browser view; Windows and Intel Mac are not validated. Read BETA.md and MACOS.md for the release gates.</footer>"""
